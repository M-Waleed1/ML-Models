...
with st.sidebar:
    ...
    st.markdown("### 🎨 Metric Color Theme")
    color_theme = st.selectbox(
        "Choose metric colors",
        ["Default (Purple)", "Blue Theme", "Green Theme", "Orange Theme", "Pink Theme", "Dark Theme"]
    )
    
    # Map theme to color style
    theme_map = {
    ...
    selected_color = theme_map[color_theme]
    
    st.markdown("---")
    st.markdown("### 📊 Model Info")
    display_metric("Accuracy", "80.4%", selected_color, "Model accuracy on test data")
    display_metric("ROC-AUC", "0.8465", selected_color, "Area under ROC curve")
    display_metric("Model Type", "Logistic Regression", selected_color, "Classification algorithm")
...
if page == "🏠 Home":
    ...
    with col1:
        display_metric("Model Accuracy", "80.4%", selected_color, "Accuracy on test set")
    
    with col2:
        display_metric("ROC-AUC Score", "84.6%", selected_color, "Area under ROC curve")
    
    with col3:
        display_metric("Features Used", "30+", selected_color, "Total features in model")
            ...
            if st.button("🚀 Run Batch Prediction", use_container_width=True):
                with st.spinner(f"Processing {len(df)} customers..."):
                    try:
                        ...
                        with col1:
                            display_metric("Total Customers", len(df_results), selected_color)
                        with col2:
...
# Analytics Dashboard Page
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("Analyzing data..."):
            ...
            with col1:
                display_metric("Total Customers", len(df_analysis), selected_color)
            with col2:
